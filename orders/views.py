...
def checkout(request, order_id):
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).error(f"Failed to create Razorpay order: {e}")
...
def payment_success(request, order_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Verify signature if order_id is present
            ...
            if order.id not in guest_orders:
                ...
                request.session['guest_order_ids'] = guest_orders
...
def my_orders(request):
    """Step 5: Customer order tracking dashboard with file viewer & real-time tracker"""
    from django.db.models import Q
    session_order_ids = request.session.get('guest_order_ids', [])
    ...
    if phone_query:
        query |= Q(customer_phone=phone_query)
...
def verify_pin(request):
    ...
    if request.method == 'POST':
        ...
        if not request.user.is_authenticated:
            messages.error(request, 'Please log in to verify order PIN.')
            return redirect('core:login')
            
        user_shops = request.user.shops.all()
        if not user_shops.exists():
            messages.error(request, 'You must be a shop owner to verify order PINs.')
            return redirect('shops:dashboard')
        
        try:
            if order_id:
                order = Order.objects.filter(id=order_id, shop__in=user_shops, pin_code=pin, status='READY').first()
            else:
                order = Order.objects.filter(shop__in=user_shops, pin_code=pin, status='READY').first()
                
            if not order:
                messages.error(request, f'Invalid PIN "{pin}" or order is not ready for pickup.')
                return redirect('shops:dashboard')

            order.complete_order()
            messages.success(request, f'🎉 Order #{order.id} verified and marked COMPLETED!')
        except Exception as e:
